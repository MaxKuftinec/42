...
#include <stdarg.h>
    ...
    #define NULL_PTR_REPRESENTATION "0x0"
#elif defined(__linux__) || defined(__unix__)
    #define NULL_PTR_REPRESENTATION "(nil)"
#else
    #define NULL_PTR_REPRESENTATION "(null)"
#endif
...
static void	ft_printnbrf(int n, int *printed_char, char type) // if type != 0, it's an unsigned
{
	if (type && n < 0)
	{
	...
	if (n < 0 && n != -2147483648)
		(n *= -1, ft_printfstrf((char[2]){'-'}, printed_char, 42));
	if (n >= 0 && n <= 9)
		ft_printfstrf((char[2]){48 + n}, printed_char, 42);
	else
    {
        ft_printnbrf(n / 10, printed_char, type); // best to put type == 0 in this line
		ft_printfstrf((char[2]){48 + (unsigned int)n % 10}, printed_char, 42);
    }
}
...
void	flag(const char *p, va_list x, int *printed_char) // Not efficient to make var in flag because we have to process vars once again
{
	...
	else if ('s' == p[1])
	{
 	   s = va_arg(x, char *);
		if (!s)
			ft_printfstrf(NULL_PTR_REPRESENTATION, printed_char, 0);
		else
...
int main()
{
	...
	//ft_putstr(str);
//	   printf("%%%c%%%s%%%d%%%i%%%u%%%x\n", 'A', "42", 42, 42, 42, 42);
//	ft_printf("%%%c%%%s%%%d%%%i%%%u%%%x\n", 'A', "42", 42, 42, 42, 42);
	
	custom = ft_printf(" %c %c %c endof\n", 0, '1', '2');
	real = printf(" %c %c %c endof\n", 0, '1', '2');
